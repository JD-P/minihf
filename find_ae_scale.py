    ...
    for epoch in trange(args.epochs, disable=not is_main):
        ...
        for input_ids, attention_mask in tqdm(dataloader, disable=not is_main):
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                input_ids = torch.split(torch.cat(torch.split(input_ids, args.vae_context, dim=1)), 8)
                attention_mask = torch.split(torch.cat(torch.split(attention_mask, args.vae_context, dim=1)), 8)
                for inputs, mask in zip(input_ids, attention_mask):
                    mean = vae_model.encode(inputs, mask)
                    z = vae_model.vae.sample(mean)
                    zs.append(z)
                if len(zs) > 100:
                    break
        zs = torch.cat(zs)
        print(torch.sqrt(torch.mean(zs ** 2)).item())
