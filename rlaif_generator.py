...
def main():
    ...
    for i in tqdm(endless_range()):
        if i % 20 == 0:
            with set_adapter(model, "default"):
        ...
        with torch.no_grad(), set_adapter(model, None):
            outputs_orig = model(tokens, attention_mask=attention_mask)
        logits_orig = at_least_float32(outputs_orig.logits)
        logp_orig = dice.logp_categorical(logits_orig[:, input_len - 1 : -1], tokens[:, input_len:])
        ...
        scores = []
        with torch.no_grad(), set_adapter(model, "evaluator"):
        ...
        scores = soft_minimum(scores, principle_weights[None], dim=1).to(logp.device)
        ...
        print(
            f"step: {i}, loss: {loss.item():g}, main: {loss_main.item():g}, kl: {loss_kl.item():g}"
        )
        ...
        if (i + 1) % args.grad_accum_steps == 0:
            ...
            grad_norm **= 0.5
            print(f"step: {i}, grad_norm: {grad_norm:g}")
            scaler.step(opt)
            scaler.update()
            sched.step()
            opt.zero_grad()
